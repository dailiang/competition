...
wc_4_file = r'C:\Users\dailiang.dl\Desktop\CIKM\Data\wc_4_trunk.txt'
...
with open( wc_4_file, 'r') as f:
	...
	for e, line in enumerate(f):
		try:
			...
			count = int(count)
			ratio1 = log(630030253. * count / wc_1[word1] * wc_3[' '.join((word2, word3, word4))])
			ratio2 = log(630030253. * count / wc_1[word4] * wc_3[' '.join((word1, word2, word3))])
			ratio3 = log(630030253. * count / wc_2[' '.join((word3, word4))] * wc_2[' '.join((word1, word2))])
			ratio.append(min((ratio1, ratio2, ratio3)))
		except:
			print 'error4', e + 1
			continue
